...
productsRouter.get("/:limit/:page/:order",async (req,res)=>  {
...
console.log(filters);
    ...
    ,    totalPages: currentProducts.totalPages
        ...
        ,prevLink: currentProducts.hasPrevPage? `http://localhost:8080/api/products/${limit}/${page-1}/${order}?category=${category.split(' ').join('%20')}&stock=${stock}`: null
        ,nextLink: currentProducts.hasNextPage? `http://localhost:8080/api/products/${limit}/${page+1}/${order}?category=${category.split(' ').join('%20')}&stock=${stock}`: null
      };
