...
viewsRouter.get("/products/:limit/:page/:order",  async (req,res)=>{
    try {
        ...
        if(parseInt(req.params.page)> currentProducts.totalPages   || parseInt(req.params.page) <1){
        ...
        let result = {status: true, totalPages: currentProducts.totalPages
            ...
            ,prevLink: currentProducts.hasPrevPage? `http://localhost:8080/products/${limit}/${page-1}/${order}?category=${category.split(' ').join('%20')}&stock=${stock}`: null
            ,nextLink: currentProducts.hasNextPage? `http://localhost:8080/products/${limit}/${page+1}/${order}?category=${category.split(' ').join('%20')}&stock=${stock}`: null
          };
